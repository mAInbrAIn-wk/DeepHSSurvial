...
============================================================================
Transformer-Architektur auf Ebene der einzelnen Prüfungen zur Vorhersage der finalen Abschlussnote.
...
def build_exam_transformer(max_exams: int, num_features: int, d_model: int = 64, num_heads: int = 4):
    ...
    # Global Pooling über alle Prüfungen
...
def main():
    print("=" * 70)
    print("ZEITREIHEN-TRANSFORMER REGRESSOR (PRÜFUNGS-EBENE)")
    print("=" * 70)
    ...
    print("\nTrainiere Keras Prüfungs-Transformer Regressor ...")
    model = build_exam_transformer(max_exams, n_features)
    model.summary()
    ...
    print("\n" + "=" * 70)
    print("ERGEBNISSE PRÜFUNGS-TRANSFORMER REGRESSOR (TEST-SET)")
    print("=" * 70)
    ...
    save_metrics("timeseries_exam_transformer", metrics_dict, output_dir)
    save_keras_model(model, "timeseries_exam_transformer", output_dir)
    plot_learning_curve(history.history, "timeseries_exam_transformer", output_dir, metric_name='mae')
    plot_parity_plot(y_test, test_preds, "timeseries_exam_transformer", output_dir)
