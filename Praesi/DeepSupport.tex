...
\begin{frame}{Methodenvergleich: Survival-Modelle (Wirkung)}
    ...
    \begin{table}[]
        ...
        \begin{tabular}{lllp{6cm}}
            ...
            \textbf{Extended Cox} & Panel (Zeitreihe) & Dynamisch (Zeitveränderlich) & \textbf{HR $\approx 0.37$} (Signifikante Entstörung) \\
            \textbf{Double ML Survival} & Panel & Orthogonalisiert (DML) & \textbf{RR $\approx 0.91$} (Realistische Effektschätzung) \\ \hline
        \end{tabular}
        }
    \end{table}
...
\textbf{Zwischenfazit:}
    \begin{itemize}
        \item Um das reaktive Confounding und das Dropout-Paradoxon aufzulösen, sind interpretierbare Panel-Modelle (Extended Cox) oder Double Machine Learning unerlässlich. 
	\item Die komplexeren rekurrenten Modelle eignen sich für diese spezifische HR-Analyse \textit{nur in Kombination mit DML}; auf Prognose ausgelegte Modelle naiv für kontrafaktische Vorhersagen zu verwenden, ist problematisch.
    \end{itemize}
\end{frame}
